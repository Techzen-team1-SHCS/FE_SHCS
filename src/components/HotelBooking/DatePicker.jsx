...
const DatePicker = ({ roomPrice = 0 }) => {
    ...
    useEffect(() => {
        if (checkIn && checkOut) {
            ...
            if (to > from) {
                ...
                const nightCount = Math.ceil(diff / (1000 * 3600 * 24));
    ...
    return (
        <div className='booking-box' style={{width:"100%"}}>
            {/* CHECK-IN & CHECK-OUT */}
            ...
            <div style={{display:"flex",alignItems:'center',justifyContent:'center', color:'white',fontSize:'30px',padding:'20px'}}>
                {totalPrice.toLocaleString("vi-VN")}$/TOTAL
            </div>
