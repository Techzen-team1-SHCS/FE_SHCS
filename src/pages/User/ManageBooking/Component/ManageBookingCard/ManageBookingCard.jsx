                    ...
                    <div className={styles.cardGroup}>
                        ...
                        <div className={styles.dateGrid}>
                            <div className={styles.dateItem}>
                                ...
                                <span className={styles.dateValue}>{formatDateTime(booking?.check_in)}</span>
                            </div>
                            <div className={styles.dateItem}>
                                <span className={styles.dateLabel}>Check-out</span>
                                <span className={styles.dateValue}>{formatDateTime(booking?.check_out)}</span>
                            </div>
                        </div>
                                        ...
                                        return (
                                            <div className="d-flex flex-column gap-2">
                                                ...
                                                <div className={styles.policyBox}>
                                                    <span className={styles.policyTitle}>Chính sách hủy phòng</span>
                                                    {cancelPolicy.hasFreeCancel ? (
                                                        <p className={styles.policyText}>
                                                            Miễn phí huỷ đến <strong>{formatDateTime(cancelPolicy.freeCancelDeadline)}</strong>. 
                                                            Sau thời điểm đó phí huỷ là {cancelPolicy.cancelFee.toLocaleString("vi-VN")} VND.
                                                        </p>
                                                    ) : (
                                                        <p className={styles.policyText}>{cancelPolicy.message}</p>
                                                    )}
                                                </div>
                                            </div>
                                        );
...
const PendingTimer = ({ createdAt, onExpire, bookingId }) => {
    ...
    if (isExpired) return <span style={{ color: 'red', fontSize: '13px' }}>⏰ Giao dịch đã hết hạn</span>;
